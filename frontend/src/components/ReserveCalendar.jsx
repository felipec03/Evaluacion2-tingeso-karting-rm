...
// Configurar el localizador proporcionando la instancia de moment al localizador
const localizer = momentLocalizer(moment);

// Para depuración - muestra eventos de ejemplo para el mes actual
...
const ReserveCalendar = () => {
    ...
    const checkBackendAndFetchData = async () => {
        ...
        try {
            // Ping simple para verificar si el backend está funcionando
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8080';
            await axios.get(`${apiUrl}/api/reservas`, { timeout: 5000 });
    ...
    const fetchReservations = async () => {
        try {
            ...
            if (!response.data || response.data.length === 0) {
                ...
                // Si no hay reservas, usamos datos de ejemplo
            ...
            // Transformar los datos en eventos que el calendario puede entender
            const calendarEvents = response.data.map(reserve => {
                console.log('Procesando reserva:', reserve);
                
                // Nos aseguramos de que las fechas son strings antes de parsearlas
                ...
                try {
                    ...
                    if (!endTime.isValid()) {
                        ...
                        endTime = moment().add(1, 'hour'); // Fallback a tiempo actual + 1 hora
                    }
                } catch (e) {
                ...
                // Convertir tiporeserva a número si es un string
                ...
                // Obtener nombre por email o usar email directamente
                const userName = reserve.emailarrendatario;
    ...
    // Función auxiliar para obtener un tipo de reserva legible
    ...
    // Estilo personalizado de eventos basado en el tipo de reserva
    const eventStyleGetter = (event) => {
        let backgroundColor = '#3174ad';
        let borderColor = '#2a5f8f';
        
        // Diferentes colores basados en el tipo de reserva
        switch(event.resource?.tiporeserva) {
            case 1: 
                backgroundColor = '#3174ad'; // Azul para normal
                borderColor = '#2a5f8f';
                break;
            case 2: 
                backgroundColor = '#5cb85c'; // Verde para extendida
                borderColor = '#4cae4c';
                break;
            case 3: 
                backgroundColor = '#f0ad4e'; // Amarillo/Naranja para premium
                borderColor = '#eea236';
                break;
            default:
                break;
    ...
    // Componente personalizado para renderizar eventos con más detalles
